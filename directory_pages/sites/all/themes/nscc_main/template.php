 ...
 * @see page.tpl.php
 */
function nscc_main_preprocess_page(&$variables) {
	...
	if (arg(0) == 'admin' || arg(0) == 'manage' || arg(2) == 'edit' || arg(2)=='delete' || arg(2)=='revisions') {
		drupal_add_css(path_to_theme().'/css/nscc-main-admin.css', 'theme');
	}
	...
	if ($node) {
		switch($node->type) {
			...
			case 'ad_frame' :
				...
				if($node->field_testi[0]){
					...
					if ($output_node->type == 'testimonial'){
						 $variables['testimonial'] .= _nscc_main_testimonial($output_node);
					}
			...
			case 'page_program_calendar' :
				...
				//left column
				$variables['left'] = _nscc_main_program_menu($node)._nscc_main_transfer_degreeslist($node)._nscc_main_program_awardslist($node).$variables['left'];
...
//						$section = ($node->field_player_gender[0]['value'] == 'Female') ? node_load(1746, null, true) : node_load(1747, null, true);
				...
				//section menu
				$variables['left'] = _nscc_main_section_menu($node).$variables['left'];
...
CONTACT_LOCATION;
			...
			case 'dept_group' :
				...
				//left sidebar
				$variables['left'] = _nscc_main_dept_group_heirarchy_menu($node) . $variables['left'];
			...
			case 'degree_proftech' :
				...
				// Left sidebar
				$variables['left'] = _nscc_main_program_menu($node)._nscc_main_program_awardslist($node) . $variables['left'];
 ...
 */
function _nscc_main_section_menu(&$node){
	switch ($node->type) {
...
*/
function _nscc_main_program_menu(&$node){
...
* Loose function to create program-related menus on page_program's and awards pages.
*/
function _nscc_main_program_awardslist(&$node){
 if($node->field_cert_program[0]['nid']){
 	$program_nid = $node->field_cert_program[0]['nid'];	
 } else {
 	$program_nid = $node->field_program[0]['nid'];	
...
*/
function _nscc_main_transfer_degreeslist(&$node){
 if($node->field_cert_program[0]['nid']){
 	$program_nid = $node->field_cert_program[0]['nid'];	
 } else {
 	$program_nid = $node->field_program[0]['nid'];	
 ...
 */
function _nscc_main_dept_group_heirarchy_menu($node){
	$output = '<div class="block block-menu"><ul class="menu">'."\n";
 ...
 */
function _nscc_main_program_testimonials(&$node){
	if($node->field_testimonials[0] || $node->field_testimonials2[0]){
		...
		if($node->field_testimonials[0]){
					...
					if ($output_node->type == 'testimonial'){
						$output .= _nscc_main_testimonial($output_node);
					}
					if ($output_node->type == 'profile'){
						$output .= _nscc_main_profile($output_node);
					}
					if ($output_node->type == 'employer_expectations'){
						$output .= _nscc_main_emp_exp($output_node);
					}
 ...
 */
function _nscc_main_testimonial(&$testimonial_node){
 ...
 *	Helper function: builds a testimonial from the specified node.
 */
function _nscc_main_emp_exp(&$employer_expectations_node){
 ...
 */
function _nscc_main_profile(&$profile_node){
 ...
 */
function nscc_main_preprocess_node(&$variables) {
	$node = $variables['node'];
	...
	switch(true) {
		...
		case ($node->type == 'program') :
			...
			if (!empty($node->field_coord_altname[0]['value'])) {
				...
				$variables['mgr'] = l(_nscc_get_nscc_profile_full_name($coordinator->uid), 'https://people.northseattle.edu/users/'.$coordinator->name, array('attributes' => array('title' => 'Contact and profile info'), 'alias' => true,));
			...
			if (!empty($node->field_contact_2_altname[0]['value']) || !empty($node->field_contact_2[0]['uid'])) {
				...
				if (!empty($node->field_contact_2_altname[0]['value'])) {
					...
					$variables['contact2'] = l(_nscc_get_nscc_profile_full_name($contact2->uid), 'https://people.northseattle.edu/users/'.$contact2->name, array('attributes' => array('title' => 'Contact and profile info'), 'alias' => true,));
		...
		case ($node->type == 'division') :
			...
			$variables['mgr'] = l(_nscc_get_nscc_profile_full_name($dean->uid), 'https://people.northseattle.edu/users/'.$dean->name, array('attributes' => array('title' => 'Contact and profile info'), 'alias' => true,));
			if ($dean->status !== 1) {
				//Notify PIO of blocked user
 ...
 */
function nscc_main_preprocess_block(&$variables) {
 ...
 */
function nscc_main_preprocess_views_view_table__committees(&$variables) {
 ...
 */
function nscc_main_preprocess_views_view__food_services__block_1(&$variables) {
 ...
 */
function nscc_main_preprocess_content_field(&$variables) {
	$node = $variables['node'];
