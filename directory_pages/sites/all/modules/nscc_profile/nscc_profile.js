// $Id: nscc_profile.js,v 1.2 2007/12/08 14:06:22 goba Exp $
 ...
 */
Drupal.behaviors.nscc_profileDrag = function(context) {
  var table = $('#nscc_profile-fields');
  var tableDrag = Drupal.tableDrag['nscc_profile-fields']; // Get the profile tableDrag object.
  ...
  tableDrag.onDrop = function() {
    ...
    if ($(dragObject.rowObject.element).prev('tr').is('.category-message')) {
      ...
      var categoryField = $('select.nscc_profile-category', dragObject.rowObject.element);
      var weightField = $('select.nscc_profile-weight', dragObject.rowObject.element);
      var oldcategoryNum = weightField[0].className.replace(/([^ ]+[ ]+)*nscc_profile-weight-([^ ]+)([ ]+[^ ]+)*/, '$2');

      if (!categoryField.is('.nscc_profile-category-'+ categoryNum)) {
        categoryField.removeClass('nscc_profile-category-' + oldcategoryNum).addClass('nscc_profile-category-' + categoryNum);
        weightField.removeClass('nscc_profile-weight-' + oldcategoryNum).addClass('nscc_profile-weight-' + categoryNum);
