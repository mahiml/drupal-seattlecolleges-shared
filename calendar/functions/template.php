...
class Page {
    ...
    // If viewing a single calendar, subscribe_path will be set, but not download_filename.
    // So set it to the same value, but replace protocol w/ 'http://'
//		if ($cal != $phpiCal_config->ALL_CALENDARS_COMBINED && $subscribe_path != '' && $download_filename != '') {		//  [MOD vellines]
		if ($cal != $phpiCal_config->ALL_CALENDARS_COMBINED && $subscribe_path != '') {
      $download_filename = str_replace('webcal://', 'http://', $subscribe_path);
...
HEREDOC;
	...
	function output() {
		...
		// Add NSCC Campus Alerts and Announcements.
		$nscc_alerts = file_get_contents('https://northseattle.edu/ajax/nsccdumpalerts');
		$this->page = str_replace('<!-- NSCC_ALERTS -->', $nscc_alerts, $this->page);
		
		// Add NSCC Global navigation.
		$nscc_nav = file_get_contents('https://northseattle.edu/ajax/nsccdumpnavindex');
		$this->page = str_replace('<!-- NSCC_GLOBAL_NAV -->', $nscc_nav, $this->page);
		
		// Add NSCC iCal list from config obj to subscribable links page.
		$this->page = str_replace('<!-- ICAL_LIST -->', format_webcals_list($phpiCal_config), $this->page);
...
// Formats list of subscribable Webcals from config array
function format_webcals_list($configs = null) {
	$ical_list = '<p>Sorry, no iCalendar formats available</p>';	// default msg.
	if ($configs && !empty($configs->nscc_calendars)) {
		foreach ($configs->nscc_calendars as $cal) {
			$cal_desc = !empty($cal['desc']) ? '<p>' . $cal['desc'] . '</p>' : '';
			$ical_items[] = '<li><a href="webcal://' . $cal['url'] . '">' . htmlentities($cal['name']) . " Calendar</a>$cal_desc</li>";
		}
		$ical_list = (count($ical_items) > 0) ? "<ul>\n" . implode("\n", $ical_items) . "</ul>\n" : $ical_list;
	}
	return $ical_list;
}
