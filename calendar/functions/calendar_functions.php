...
//	print "\n\nCal-Path: $cal_path\n\n";
	...
	// Pull name from config array, if possible.
	$cal_url = parse_url($cal_path);
	if ($cal_url !== false) {
		foreach ($phpiCal_config->nscc_calendars as $cal_meta) {
			if ($cal_meta['url'] == $cal_url['host'].$cal_url['path']) {
				return $cal_meta['name'];
			}
		}
	}
