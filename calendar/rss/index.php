...
$current_view = "rss_index";
$display_date = "Subscribable Formats";

$rss_list = "<table>\n";
$xml_icon ='<img src = "feed.png" alt="rss icon" />';
$cals[] = '';
$cal_displaynames[] = $all_cal_comb_lang;
//$debug[] = '$cals[] =' . print_r($cals, true);
//$debug[] = '$cal_displaynames[] = ' . print_r($cal_displaynames, true);
foreach ($cals as $k=>$file) {
	...
	// Pull calendar name from config, if available.
	$calname = $cal_displaynames[$k];
	foreach ($phpiCal_config->nscc_calendars as $webcal) {
		$calname = ($calname == $webcal['cachefile']) ? $webcal['name'] : $calname;
	}
	
//	$rss_list .= '<tr><td rowspan ="3"><h3>'.$cal_displaynames[$k].' '. $lang['l_calendar'].'</h3></td>';
	$rss_list .= '<tr><td rowspan="3"><h3>'.$calname.' '. $lang['l_calendar'].'</h3></td>';

/* Changed to show links without urlencode, but links valid urls */
	$rss_list .= '<td><label>'.$lang['l_day'].'</label></td>';
	...
	$rss_list .= '<tr>';
	$rss_list .= '<td><label>'.$lang['l_week'].'</label></td>';
	...
	$rss_list .= '<tr>';
	$rss_list .= '<td><label>'.$lang['l_month'].'</label></td>';
