...
$search_box .=
	...
	'<label>'.$lang['l_search'].':</label>'.
	'<input type="text" class="query-field" name="query" value="'.$query.'">'."\n".
	'<input type="image" class="search-button" src="templates/'.$phpiCal_config->template.'/images/search.gif" name="submit" value="Search">'."\n".
	'</form>'."\n";
...
if ($search_valid) {
	...
	if (!$format_search_arr[0]) {
		$formatted_search = $no_query_lang;
	} else {
	...
	$formatted_search = $lang['l_no_query'];
}
...
function format_search($search_str) {
	...
	for($i=0;$i<$count;$i++) {
		if ($i != 0 && $and_arr[$i] == 'or') {
			...
			$or_str_arr[$j] = implode('</span> or <span class="search-term">', $or_arr[$j]);
			$j++;
		}
	}
	...
	if (count($and_arr) > 1) {
		$final_str_arr[] = implode('</span> and <span class="search-term">', $and_arr);
	} elseif (isset($and_arr[0]) && $and_arr[0] != '') {
		$final_str_arr[] = $and_arr[0];
	}

	if (count($or_str_arr) > 1) {
		$final_str_arr[] = implode('</span> and <span class="search-term">', $or_str_arr);
	} elseif (isset($or_str_arr[0]) && $or_str_arr[0] != '') {
		$final_str_arr[] = $or_str_arr[0];
	}

	if (count($not_arr) > 1) {
		$final_str_arr[] = '-'.implode('</span> and <span class="search-term">-', $not_arr);
	} elseif (isset($not_arr[0]) && $not_arr[0] != '') {
		$final_str_arr[] = '-'.$not_arr[0];
	}

	if (count($final_str_arr) > 1) {
		$formatted_search = '<span class="search-term">'.implode('</span> and <span class="search-term">', $final_str_arr).'</span>';
	} else {
		$formatted_search = '<span class="search-term">'.$final_str_arr[0].'</span>';
	}
